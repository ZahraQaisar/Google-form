  ...
  useEffect(() => {
    setDynamicResponses((prev) => {
      ...
      dynamicFields.forEach((q) => {
        updated[q.id] = prev[q.id] ?? ""; // keep old answers or initialize new
      });
      return updated;
    });
  }, [dynamicFields]);
...
const handleSubmit = async (e) => {
  ...
  const payload = {
    formType: "Quiz",
    user: username,  // top-level field
    capitalAnswer,
    mathAnswer,
    planetAnswer,
    ...dynamicResponses, // dynamic questions flattened into payload
  };
                    ...
                    {field.options?.map((opt, idx) => (
                      <label key={idx} className={`flex items-center space-x-2 ${theme === "dark" ? "text-gray-100" : "text-gray-900"}`}>
                        <input
                          ...
                          name={`mcq-${field.id}`}
                          value={opt}
                          checked={dynamicResponses[field.id] === opt}
                          onChange={() => handleDynamicChange(field.id, opt)}
                        />
                        <span>{opt}</span>
                      </label>
                    ))}
